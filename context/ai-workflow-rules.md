...
- Separate map/GeoJSON rendering work from ubicaciones catalog/data-model work — one is presentation, the other is the data source it depends on.
...
Role gating (`transportista`/`empresa`), the paid-listing invariant, and the notification triggers all cut across multiple files. Any change touching one of the following must be checked against all of the others before it's considered done:
