...
- Prefer `interface` for object shapes that may be extended (e.g. a `Carga` base shape used across variants); use `type` for unions, intersections, and aliases.
...
- Auth and ownership are enforced before any write: confirm the session, confirm the role matches the action (e.g. only `empresa` can publish a carga), confirm the acting user owns the resource being modified.
...
## Data and Storage
- Structured entities — `users`, `viajes`, `cargas`, `ubicaciones`, `postulaciones`, `payments` — live in Postgres via the ORM; no structured entity is stored as unstructured JSON blobs without reason.
...
- If using Supabase Postgres, enforce Row Level Security policies scoped by `auth.uid()` and `role` as the data-layer backstop to app-level auth checks — see architecture-context.md, Auth and Access Model.
...
- `components/{domain}/` — composed domain components (e.g. `components/carga/carga-card.tsx`, `components/viaje/viaje-form.tsx`).
- `content/` or `lib/i18n/` — Spanish copy/dictionary files.
