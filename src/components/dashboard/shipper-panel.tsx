...
function SpendCard() {
  ...
  return (
    <CardPanel className="p-6">
      <CardEyebrow>{spend.eyebrow}</CardEyebrow>
      ...
      <dl className="mt-[18px] flex flex-col gap-2.5 border-t border-line-subtle pt-4 text-[13.5px]">
        {spend.breakdown.map((row) => (
          <div key={row.label} className="flex justify-between gap-4">
            ...
            <dd className={cn("m-0", "positive" in row && row.positive && "text-status-published-text")}>
              {row.value}
            </dd>
          </div>
        ))}
...
function PackOfferCard() {
  const { packOffer } = panel;

  return (
    <CardPanel tone="warm" className="p-[22px_24px]">
      <CardEyebrow tone="brand">{packOffer.eyebrow}</CardEyebrow>
      <p className="mt-3 text-sm leading-[1.6] text-ink-muted">{packOffer.body}</p>
...
function NextExpiryCard() {
  const { nextExpiry } = panel;

  return (
    <CardPanel tone="sunken" className="p-[22px_24px]">
      <CardEyebrow>{nextExpiry.eyebrow}</CardEyebrow>
      <div className="mt-3 text-sm font-semibold">{nextExpiry.title}</div>
      <p className="mt-1.5 text-[13.5px] leading-[1.55] text-ink-subtle">{nextExpiry.body}</p>
    </CardPanel>
  );
}
